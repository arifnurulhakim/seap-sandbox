...
// do something with the $afk_data array
        ...
        $absensi="";
            if($user->divisi_id != 4){
                ...
                ->select('users.name','users.id as user_id','absensis.id as absen_id', 'absensis.start_day','absensis.end_day','absensis.status','absensis.created_at')
                    ...
                    ->wherenotin('users.id',DB::raw('SELECT user_id FROM pis_users_excludes'))
                    ->get();
    ...
    foreach ($absensi as $row) {
        ...
        if($row->start_day){
            ...
            //     $total_afk = DB::table('absensis')
            // ->leftJoin('afks', function ($join) {
            //     $join->on('absensis.user_id', '=', 'afks.user_id')
            //          ->whereRaw('pis_absensis.start_day <= pis_afks.end_afk')
            //          ->whereRaw('pis_absensis.end_day >= pis_afks.start_afk');
            // })
            // ->where('absensis.user_id', '=', $row->user_id)
            // ->where(function ($query) {
            //     $query->whereNull('afks.start_afk')
            //           ->orWhere(function ($query) {
            //               $query->whereNotNull('afks.start_afk')
            //                     ->whereRaw('pis_absensis.start_day <= pis_afks.start_afk')
            //                     ->whereRaw('pis_absensis.end_day >= pis_afks.end_afk');
            //           });
            // })
            // ->select('absensis.start_day','absensis.end_day', DB::raw('COALESCE(pis_afks.start_afk, NULL) AS start_afk'), DB::raw('COALESCE(pis_afks.end_afk, NULL) AS end_afk'),DB::raw('SUM(TIME_TO_SEC(TIMEDIFF(COALESCE(end_afk, "' . $end_afk . '"), start_afk))) AS total_durasi_afk'))
            // ->groupby('absensis.start_day','absensis.end_day','start_afk','end_afk')
            // ->first();
            // dd($total_afk);


            $total_afk_durasi = $total_afk->total_durasi_afk;
            if($total_afk){
               $total_afk ="0.000000";
            }
                ...
                $total_waktu_kerja = strtotime($end_day) - strtotime($row->start_day);
                $total_waktu_afk = $total_afk ? $total_afk_durasi : 0;
