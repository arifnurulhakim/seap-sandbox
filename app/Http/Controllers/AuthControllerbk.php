...
// AuthController.php
    ...
    public function getUserBydivisi()
    {
        ...
        $users = User::leftJoin('roles', 'users.role_id', '=', 'roles.id')
            ...
            ->select('users.id', 'users.name', 'users.join_date', 'users.email', 'roles.name as roles_name', 'divisis.name as divisis_name',
            ...
            ->where(function ($query) use ($role, $divisi, $cek_user_ex,$getdivisi,$user) {
                if($cek_user_ex){
        ...
        foreach ($users as $u) {
            ...
            $periode = NilaiEvaluasiUser::select('periode')
