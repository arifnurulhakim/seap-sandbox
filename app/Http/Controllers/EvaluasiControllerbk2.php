...
public function result($user_id){
    ...
    $nilaiRataRata = NilaiEvaluasiUser::select('periode',
...
foreach ($nilaiRataRata as $data) {
    ...
    foreach ($dataLead as $z => $lead) {
        ...
        // dd($nilaievaluasiuser1);
        // $allNilaiNull = true;
        // $allNilaiNull2 = true;

        // foreach($nilaievaluasiuser1 as $nilai) {
        //     if($nilai->nilai != null) {
        //         $allNilaiNull = false;
        //         break;
        //     }
    ...
    foreach ($feedback as $fc) {
        $data_feedback['periode_'.$fb] = $fc->periode ?? null;
        $data_feedback['label_'.$fb] = $fc->label ?? null;
        $fb1=1;
        $fb2=1;
        foreach ($feedback1 as $fc1){
            
            $data_feedback['reviewer_id_'.$fb.'_'.$fb1] = $fc1->reviewer_id ?? null;
            $data_feedback['reviewer_name_'.$fb.'_'.$fb1] = $fc1->reviewer_name ?? null;

            $reviewer_name_fb_arr1 = explode(" ", $fc1->reviewer_name ?? '');
            $reviewer_fb_inisial1 = '';
            foreach ($reviewer_name_fb_arr1 as $nama_fb1) {
                $reviewer_fb_inisial1 .= substr($nama_fb1, 0, 1);
            }

            $data_feedback['reviewer_inisial_'.$fb.'_'.$fb1] = strtoupper($reviewer_fb_inisial1);
        
            
            $data_feedback['feedback_'.$fb.'_'.$fb1] = $fc1->feedback ?? null;
            $fb1++;
        }
        foreach ($feedback2 as $fc2){
            $data_feedback['reviewer_id_'.$fb.'_'.$fb2] = $fc2->reviewer_id ?? null;
            $data_feedback['reviewer_name_'.$fb.'_'.$fb2] = $fc2->reviewer_name ?? null;
            ...
            $data_feedback['reviewer_inisial_'.$fb.'_'.$fb2] = strtoupper($reviewer_fb_inisial2);
        
            $data_feedback['feedback_'.$fb.'_'.$fb2]= $fc2->feedback ?? null;
            $fb2++;
        }
        $fb++;
    }
...
public function result2($user_id){
    ...
    if (!$user) {
        ...
        $review_user = User::where('id', $user_id)->first();
        $getname = $review_user->name;
        $getdivisi = $review_user->divisi_id;
        $getdivisiName = Divisi::select('name')->where('id',$getdivisi)->first();
        $getdivisiName = $getdivisiName->name;
        $getrole = $review_user->role_id;
        // $role ='';
        // if ($review_user->role_id == 4){
        //     $role = 3;
        // };
    ...
    $dataLead = DB::table('nilai_evaluasi_users')
        ->select('nilai_evaluasi_users.reviewer_id', 'users.name')
        ->distinct()
        ->where('nilai_evaluasi_users.user_id', '=', $user_id)
        ->leftJoin('users', 'users.id', '=', 'nilai_evaluasi_users.reviewer_id')
    ...
    $nilaiRataRataWithReviewers = [];
    foreach ($nilaiRataRata as $data) {
        $reviewerName = isset($reviewers[$data->reviewer_id]) ? $reviewers[$data->reviewer_id] : 'Unknown';
        $nilaiRataRataWithReviewers[] = [
            'periode' => $data->periode,
            'label' => $data->label,
            'nilai_rata_rata' =>  number_format($data->nilai_rata_rata, 1, '.', ''),
        ...
        foreach ($dataLead as $z => $lead) {
            $nilaiRataRataWithReviewers[count($nilaiRataRataWithReviewers) - 1]["reviewer_name_" . ($z + 1)] = $lead->name;
        }
    }
        ...
        ->where('evaluasis.divisi_id', '=', $getdivisi)
        ->select('evaluasis.id as id_evaluasi', 'nilai_evaluasi_users.*', 'nilai_evaluasi_users.nilai as nilai','users.name as reviewer_name')->get();
    ...
    foreach ($feedback as $fc) {
        ...
        foreach ($feedback1 as $fc1){
            ...
            $data_feedback['reviewer_inisial_1_'.$fb.'_'.$fb1] = strtoupper($reviewer_fb_inisial1);
        ...
        foreach ($feedback2 as $fc2){
            ...
            $data_feedback['reviewer_inisial_2_'.$fb.'_'.$fb2] = strtoupper($reviewer_fb_inisial2);
            ...
            foreach ($nilaievaluasiuser1 as $nilai1) {
                ...
                foreach ($nilaievaluasiuser2 as $nilai2) {
                    if ($nilai2->id_evaluasi == $eva->id) {
                        ...
                        $reviewer_name_arr2 = explode(" ", $nilai1->reviewer_name ?? '');
                        $reviewer_inisial2 = '';
                        foreach ($reviewer_name_arr2 as $nama2) {
                            $reviewer_inisial2 .= substr($nama2, 0, 1);
                        }
